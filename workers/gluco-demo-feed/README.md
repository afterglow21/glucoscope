...
A new browser session showed `公開デモ · ライブデータ`, three available and selected controls, the three-source chart message, and Guardian, Libre, and G7 cards without an update-delay state. The same tab completed its five-minute automatic refresh with the live state and all three sources preserved; the Libre displayed-point aggregate changed from 526 to 525, and no console error was observed. This verifies multiple scheduled aggregate checks and one browser auto-refresh. Longer, repeated browser auto-refresh observation and production natural expiry remain unobserved.
...
25. Keep the general-user Limited Data Relay paused unless it receives its own separate approval.
