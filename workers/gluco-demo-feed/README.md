...
This Worker is a public-demo-only, multi-source feed for Kazuma's data. It is separate from the general-user Limited Data Relay. Publication consent is currently recorded only for Libre; G7 remains a stopped, unverified source until separate publication consent and end-to-end verification are complete.
...
- The existing Libre Secret values remain Cloudflare-only. The new G7 names `GLUROO_DEMO_G7_SOURCE_URL` and `GLUROO_DEMO_G7_API_SECRET` are declared locally, but their values have not been registered in Cloudflare.
...
- `GET /v1/dexcom-g7` is locally prepared but has not been deployed or activated in the frontend. No revised Worker Version, binding, traffic allocation, Secret value, or KV value was created or changed in Cloudflare for G7.
...
- Public visitors read only the source-specific KV snapshot through `/v1/libre` or `/v1/dexcom-g7`. They never cause a Gluroo request and never receive a Gluroo URL or API Secret.
...
9. Before G7 publication, obtain separate explicit consent for Kazuma's G7 glucose values and measurement/update timing.
10. After separate explicit approvals, register only the two G7 Secret values and deploy a stopped revision. Do not combine either operation with live enablement.
11. Verify the stopped G7 GET, approved-origin CORS and preflight, unapproved-origin rejection, Secret names, bindings, absent G7 KV value, and Cron exit before G7 Secret access or KV access.
12. After another separate explicit approval, enable only the intended source. Confirm one scheduled refresh, inspect only the sanitized public response, and verify that all Gluroo URLs and API Secrets are absent.
13. Keep the general-user Limited Data Relay paused unless it receives its own separate approval.
