...
This Worker is a public-demo-only, multi-source feed for Kazuma's data. It is separate from the general-user Limited Data Relay. Publication consent is recorded separately for Kazuma's Libre and G7 glucose values and measurement/update timing. G7 completed one temporary scheduled retrieval, source-specific KV write, and sanitized public Worker-response check. Both source routes are currently returned to the stopped state; G7 frontend use and the comparison-page end-to-end path remain unverified.
...
Dexcom G7
        ...
        -> one verified scheduled G7 source refresh
        -> public:dexcom-g7:v1 (retained until existing TTL)
        -> Worker response verified; public comparison remains pending
```

## Checked-in safety state

- `DEMO_FEED_ENABLED=false`, `DEMO_LIBRE_FEED_ENABLED=false`, and `DEMO_G7_FEED_ENABLED=false`.
- The dedicated `DEMO_FEED_CACHE` KV namespace was created after explicit approval on 2026-08-06, and its non-secret namespace id is recorded in `wrangler.jsonc`. It was empty through the stopped-deployment checks. The later temporary G7 acceptance created one G7 key, which is not served while paused and remains only until its existing TTL expires.
...
- `DEMO_G7_FEED_CACHE_KEY=public:dexcom-g7:v1` is a non-secret, source-specific KV key. One temporary G7-only check wrote this key. The raw KV value was not directly read or printed; the key is retained only until its existing 36-hour expiry.
- `GET /v1/dexcom-g7` completed one sanitized live-response check and is now stopped again without frontend activation. Both `/v1/libre` and `/v1/dexcom-g7` currently return `503 demo_feed_paused`; approved-origin G7 preflight returns `204`, and an unapproved Origin returns `403`.
...
Deployment `8de64190-7558-43c6-83c1-1e29a2cf80de` then restored stopped Version `9994a142-a4ca-4885-9077-952ec8e7e8d2` at 100% with all three gates `false`. Both routes again return `503`. After the next stopped Cron boundary, KV metadata still listed only the G7 key and its expiration was unchanged, so the stopped Cron neither refreshed the snapshot nor extended its lifetime. The retained key is not served while paused and will expire under its existing TTL. The frontend G7 endpoint remains blank and `dexcomRouteVerified=false`.
...
15. After another separate explicit approval, activate and verify the G7 frontend and public comparison-page end-to-end path. Repeated refreshes, stale/fallback/expiry behavior, and continuing enablement remain separate checks.
16. Keep the general-user Limited Data Relay paused unless it receives its own separate approval.
