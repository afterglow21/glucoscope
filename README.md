...
## Plus 30-day pass — stopped non-public staging checkpoint
...
- `workers/gluco-plus-entitlement/` is a non-public, disabled foundation. It includes short-code email-account and recovery routes, account deletion for accounts without purchase records, a test-mode Stripe Checkout/Webhook adapter, and a server-side guard that reuses or blocks an unfinished Checkout instead of creating a second payable session. The matching settings UI is also checked in but hidden.
- Code checkpoint `b5669df` is deployed only as the stopped `glucoscope-plus-entitlement-staging` Worker. Version `bbc6c159-ce64-4fbf-a120-a43f9c5ca5d9` receives 100% of that Worker's traffic, but `workers_dev=false`, preview URLs are off, there are no routes or Cron triggers, observability is off, and the live `workers.dev` URL returns `404`. The Version has no Secrets and every account, cleanup, RPC, purchase, Checkout, webhook, sales-readiness, and tax-readiness switch remains `false`.
- The staging-only D1 database `glucoscope-plus-staging` exists in APAC. Migrations `0001` through `0005` are applied, and all 12 application tables were verified at zero rows after deployment. Request-code and verify bindings use staging-specific rate-limit IDs, distinct from the future production IDs, with limits of 5 and 30 attempts per 60 seconds. Because account auth is off, those bindings are not used. The connecting IP is designed only as a limiter key and is not saved to D1 or application logs.
...
- `auth.glucoscope.app` is verified for sending in Resend after the required SPF, DKIM, MX, and DMARC records were added manually in Cloudflare DNS and resolved publicly. Receiving is off and open/click tracking is not configured. Closed-test credentials were used only by the non-public candidate; their values are not recorded here and the stopped traffic-serving Version exposes none of them.
- The Plus D1 binding exists only in the stopped staging environment. No Stripe key, Webhook Secret, Price/Product identifier, public account route, or live purchase path is configured. Every public account, purchase, quota, and feature switch remains `false`, so no public email or payment request can start. Public account access and sales remain no-go until the retention exception, recovery and delivery-failure procedures, terms, tax, support, payment tests, and production acceptance pass.
...
## Long-lived Gluroo device session — checked-in migration candidate

- The checked-in candidate replaces the one-hour JavaScript-readable relay ticket with an anonymous device session at `https://relay.glucoscope.app`, called only from `https://glucoscope.app`. It has not been deployed or published. No DNS, Pages, Worker Version, route, or live traffic was changed by this source update.
- The relay sets a host-only `__Host-glucoscope_relay_session` cookie with `Secure`, `HttpOnly`, `SameSite=Strict`, and `Path=/`. JavaScript never receives or stores the raw session token. The candidate contains no legacy ticket endpoint or acceptance path; the existing Cloudflare ticket Version remains the rollback artifact until the coordinated migration is accepted.
...
- Existing early-access connections intentionally require one new safety check after migration. On iPhone, add GlucoScope to the Home Screen first, open the icon, and make the initial connection there: WebKit can copy cookies into the Home Screen app but does not copy local storage. See [WebKit's Home Screen web app storage note](https://webkit.org/blog/14787/webkit-features-in-safari-17-2/).
...
The Gluroo relay accepts glucose entries only. It does not retrieve treatments, insulin, carbohydrates, medication, pump settings, or device-status data. The Gluroo URL, token, and glucose response pass transiently through Cloudflare infrastructure and the relay Worker, but the application does not store, cache, log, send to AI, or share those raw values. The currently deployed early-access Version still uses the historical ticket/counter design; the unpublished migration candidate uses the anonymous per-device record described above. A proposed replacement must return at least one valid glucose entry before the new device session replaces the existing one, so a typo, an empty source, or an upstream failure does not discard a working connection. After migration, rollback uses a reviewed stopped Version of the new device-session code; the old ticket Version is retained only as historical evidence.
...
Phase 3A connected the user onboarding flow to the paused relay client. Phase 3B created the paused Worker shell and SQLite Durable Object in Cloudflare, registered the required Worker Secrets, and passed stopped-response/CORS smoke tests. The currently deployed historical Version still uses the approved `workers.dev` target and explicit consent. Production deployment `5f8d00d9-9d68-4b2a-99cd-c58c26123684` currently routes 100% to accepted Version `a398d59e-54c1-4b8d-a9a4-b779af360a54` for the 1–3 person early-access group; stopped Version `635b8ad5-0c0e-49ff-a8c3-5dc3e8704a0a` remains its rollback target. The checked-in, unpublished candidate instead targets only `relay.glucoscope.app`, has no ticket compatibility path, uses a plain relay explanation without the separate checkbox, and keeps `RELAY_ENABLED=false`, `RELAY_DEVICE_SESSIONS_ENABLED=false`, `workers_dev=false`, Preview URLs off, and observability off. Usage deployment `4fbf0e2c-5f5c-4f4f-98a9-ae57d73b4824` routes accepted Version `5d160aed-7b27-48e6-b0a8-783534f97b6f` at 100%, with initial D1 counts `0 / 0 / 0`. Direct Nightscout and the separately consented public demo remain independent.
...
8. Current live state until migration: keep the historical ticket relay enabled only for the approved small group, with its reviewed stopped Version ready for immediate rollback.
9. Checked in but not released: accept the long-lived device-session Worker, custom domain, new-origin site, one-time reconnect, Home-Screen-first iPhone flow, 180-day idle expiry, deletion, and per-device/global limits before any coordinated migration.
...
11. Return to Worker usage-counter and Usage Dashboard production verification, the site-wide Trust/About review, feedback, and the first announcement.
