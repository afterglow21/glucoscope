...
- A provider HTTP `200` means acceptance for a delivery attempt, not proof that the intended inbox received the message. The operator must confirm the real inbox in the closed test, review bounce and spam-complaint rates daily, keep them below 4% and 0.08%, and pause local sending before those boundaries are reached. No fixed provider requests-per-second value is canonical here; the real Resend Usage page, response rate-limit headers, and `429`/`retry-after` take precedence.
- `auth.glucoscope.app` is verified for sending in Resend after the required SPF, DKIM, MX, and DMARC records were added manually in Cloudflare DNS and resolved publicly. Receiving is off and open/click tracking is not configured. No Resend API key or related Worker Secret exists, and no real email has been sent.
- The Plus D1 binding exists only in the stopped staging environment. No Resend API Secret, Stripe key, Webhook Secret, Price/Product identifier, public account route, or live purchase path is configured. Every account, purchase, quota, and feature switch remains `false`, so no email or payment request can start. Public account access and sales remain no-go until a real closed email test and explicit acceptance of the ordinary 30-day retention and longer suppression-list exception pass.
- AI quota and Plus feature-gating code are also checked in disabled. Existing AI and custom-range behavior therefore remain unchanged until identity, recovery, refunds, tax, support, public-demo anti-bypass handling, closed email acceptance, and production acceptance are complete.
