...
- Both APAC D1 databases now exist. `glucoscope-plus-staging` and `glucoscope-plus-production` have migrations `0001` through `0007`; all 13 application tables in each were verified at zero rows. Production stopped Version `0cbbc227-6da0-41c3-9fd5-f280cd291152` is deployed alone at 100% with the production D1 and account rate-limit bindings, dedicated production Turnstile, Resend, and restricted live Stripe Secret bindings, the two account HMAC Secret bindings, explicit Stripe live mode, the exact live Product/Price IDs, and the approved JPY 400 sale terms. Every account, cleanup, RPC, purchase, Checkout, webhook, sales, tax, and receipt flag is false. The production-only Custom Domain `plus.glucoscope.app` has no Cron or `workers.dev` target and returns the stopped `503` response with `no-store`. A Dashboard Secret update briefly deployed candidate-derived Version `7e823112-d8c1-4f8d-b563-101641169ce8` with account HTTP enabled while every commerce flag remained false; it was detected, replaced first by the prior stopped Version, and then by the reviewed current stopped Version. A post-correction read found all 13 production application tables at zero rows. Version `0cbbc227-6da0-41c3-9fd5-f280cd291152` is the only reviewed production rollback point; Version `7e823112-d8c1-4f8d-b563-101641169ce8`, unserved Secret Version `0b4842a9-9a62-4705-8358-612f926ad767`, and every earlier production Version must not receive traffic.
