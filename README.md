...
The local multi-source revision declares only the new G7 Secret names `GLUROO_DEMO_G7_SOURCE_URL` and `GLUROO_DEMO_G7_API_SECRET`, the separate `public:dexcom-g7:v1` KV key, and the stopped `/v1/dexcom-g7` route. No G7 Secret value was registered, no G7 KV value was written, no revised Worker Version was deployed, no Cloudflare binding or traffic allocation changed, and no G7 frontend endpoint was activated. Live retrieval still requires separate explicit approvals. Raw exports, credentials, and unreviewed candidate files remain out of Git.
...
- General-user Gluroo connection details may stay in the selected browser and pass transiently through the limited relay. They are not stored in Azure, KV, Durable Objects, relay logs, shared cache, or AI. The separate public demo feed is limited to Kazuma's explicitly consented demo values and never receives a general user's connection details or glucose data. Consent is currently recorded only for Libre; the locally prepared G7 slot remains disabled and unpublished.
