...
- On 2026-08-17, Stripe test mode created one active `GlucoScope Plus 30日パス` Product with one default JPY 400 one-time Price. The test Product/Price identifiers are present only in an unserved, fully stopped staging candidate; no recurring Price, Checkout, payment, Stripe API key, webhook Secret, route, or sale was enabled.
...
- The staging-only D1 database `glucoscope-plus-staging` exists in APAC. Migrations `0001` through `0006` are applied. The fail-closed `0006` migration replaced only the empty JPY 300 constraints with JPY 400 constraints, and all 12 application tables were verified at zero rows afterward. Request-code and verify bindings use staging-specific rate-limit IDs, distinct from the future production IDs, with limits of 5 and 30 attempts per 60 seconds. Because account auth is off, those bindings are not used. The connecting IP is designed only as a limiter key and is not saved to D1 or application logs.
- Unserved staging Version `a0805f46-8585-47c5-b431-dfcb463d2993` contains the JPY 400 code and the two non-secret Stripe test identifiers with every release/readiness flag still `false`. It receives no traffic. The older stopped Version `bbc6c159-ce64-4fbf-a120-a43f9c5ca5d9` remains at 100%, so this checkpoint did not change routing, Cron, public reachability, email, or payment behavior.
