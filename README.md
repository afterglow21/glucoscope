...
- The Plus D1 binding exists only in the stopped staging environment. The Stripe webhook destination is disabled, its temporary Custom Domain has been deleted, and there is no public account route, Checkout, Cron, or live purchase path. Every public account, purchase, quota, and feature switch remains `false`, so no public email or payment request can start. The checked-in candidate now has a visible final order review instead of a short browser prompt, a non-personal Stripe payment description, and a separate fail-closed receipt-email acceptance gate. Public account access and sales remain no-go until additional small-inbox acceptance, real delayed-delivery handling, the retention exception, actual Stripe receipt-email acceptance, professional review, acceptance of any additionally enabled payment method, and production acceptance pass.
