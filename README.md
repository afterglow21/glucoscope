...
- Both APAC D1 databases now exist. `glucoscope-plus-staging` and `glucoscope-plus-production` have migrations `0001` through `0007`; all 13 application tables in each were verified at zero rows. Production stopped Version `b81b0833-7948-48bd-8b99-88b6eb5f4845` is deployed at 100% with the production D1 binding, both account rate-limit bindings, newly generated email-lookup and verification-code HMAC Secret bindings, and the approved JPY 400 sale terms. Every release flag remains false. The production-only Custom Domain `plus.glucoscope.app` is provisioned with no Cron or `workers.dev` target; an allowed-Origin preflight returned the stopped `503 service_unavailable` response with `no-store`, without reading a request body or writing D1. This Version remains the only reviewed production rollback point. Earlier production Versions, including `c3a49718-e961-4557-b2bf-5f9224e8225f` and the unserved unusable-key Version `7dc440a4-6fc5-4907-8590-9c2b3ef97b3f`, must not receive traffic.
