...
- The staging-only D1 database `glucoscope-plus-staging` exists in APAC. Migrations `0001` through `0007` are applied. The fail-closed `0006` migration replaced only the empty JPY 300 constraints with JPY 400 constraints. Migration `0007` adds only the six-field irreversible Share Studio trial reuse-retention table and its expiry index. All 13 application tables were verified at zero rows afterward. Request-code and verify bindings use staging-specific rate-limit IDs, distinct from the future production IDs, with limits of 5 and 30 attempts per 60 seconds. Because account auth is off, those bindings are not used. The connecting IP is designed only as a limiter key and is not saved to D1 or application logs.
