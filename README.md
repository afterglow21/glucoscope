...
- On 2026-08-21, the user-facing Share Studio was replaced with a four-image carousel aligned with the protected administrator Social Share Studio. The cover uses the exact Gluco encountered in that browser on that day rather than a selector; the remaining slides contain daily metrics and graph, a gentle reflection, and a stable closing card. All four PNGs must be written to and read back from browser IndexedDB before the free trial is completed. Closing the dialog no longer loses the result: saved slides can be reopened, shared, or saved again until explicitly deleted or browser data is cleared. The four images and glucose data remain on the device and are not sent to the Plus Worker.
