...
The comparison lab is published, but it currently falls back to the clearly labelled synthetic dataset because the dedicated public-demo Worker is paused. The Libre feed has completed one scheduled Worker retrieval and sanitized public-response check and was returned to the stopped Version. Guardian's separate Azure Nightscout browser route is verified, but that does not make the combined comparison page live by itself. The published frontend now configures the stopped G7 endpoint with `dexcomRouteVerified=false`; GitHub Pages loaded the clearly labelled `準備中 · 合成データ` fallback and all three device cards after commit `8b0481a`. The lab does not rank devices, claim accuracy, select a reference CGM, or support treatment decisions. PR #14 merged the original comparison frontend to `main` in merge commit `7e96648c27ce20fabe2f283c384124e36ce0b2d2`.
...
3. Current, with separate operational approval: briefly enable the required demo feeds and verify Guardian, Libre, and G7 together on GitHub Pages.
...
- General-user Gluroo connection details may stay in the selected browser and pass transiently through the limited relay. They are not stored in Azure, KV, Durable Objects, relay logs, shared cache, or AI. The separate public demo feed is limited to Kazuma's explicitly consented Libre and G7 demo values and never receives a general user's connection details or glucose data. The Libre and G7 slots have each completed one temporary live Worker check and have been returned to the stopped Version. The published frontend configures the stopped G7 URL with `dexcomRouteVerified=false`, and its GitHub Pages synthetic fallback check passed.
