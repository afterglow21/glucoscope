...
The current AI Worker atomic-counter checkpoint is Version `c0a31ac7-257c-4225-a8f1-3bf7669f6937` at 100%. The only reviewed direct rollback after atomic activation is the unserved atomic stopped Version `46f44888-002b-4847-8553-5cd12e3d7ac5`. Historical Version 29 (`235cdf03-31d7-40fd-ab58-5c1c6aa2d923`) and deployment `a5b57a76-954b-4bb9-bbba-c23bfd0fa516` record the earlier personal-user AI boundary release. Personal-user AI remains enabled for the small early-access group under the boundary below.

## Canonical current-state snapshot — 2026-08-16 JST
...
- AI Worker atomic-counter Version `c0a31ac7-257c-4225-a8f1-3bf7669f6937` receives 100% of AI traffic. Unserved atomic stopped Version `46f44888-002b-4847-8553-5cd12e3d7ac5` is the only reviewed direct rollback. The old `7ea0cfef-5322-4370-b72d-e2885f129f38`, Phase A, and pre-activation quiesce Versions must not receive rollback traffic after the atomic schema marker was written.
- Usage Worker Version `e7b2a895-c418-4cb2-b565-d2a37bef8e1b` receives 100% of Usage traffic for the approved small early-access group. Unserved stopped Version `e1496203-ab4b-429f-acd3-4e862cff0c2f` is the reviewed direct rollback. The checked-in configuration remains fail-closed.
- The Usage-to-AI service-binding aggregate is live. Because fewer than 10 consenting device profiles contributed in the completed 30-day window, the current response is `suppressed` and contains no exact totals. Backend `GET` acceptance passed; the public Dashboard's real-browser visual check and one supervised real AI generation with an exact counter delta remain pending.
...
- The morning, afternoon, and night limits of 10 each and the daily maximum of 30 are one infrastructure-wide counter shared by the public demo and all users. They are not individual allowances. The live Worker now reserves capacity atomically before provider work and completes or releases the same idempotent reservation afterward, so simultaneous requests cannot overwrite the whole counter snapshot.
...
11. Current follow-up: verify the Worker usage counter and Usage Dashboard in production, finish the connection/manual feedback work, complete the site-wide Trust/About review, and complete Plus sales blockers before the intended simultaneous first announcement and Plus launch.
