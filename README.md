...
- Separate staging and production Plus D1 databases now exist. The production Custom Domain serves only the reviewed signature-verified Stripe webhook; public account, Checkout, and Share routes remain stopped, there is no Cron or `workers.dev` route, and every public sale and feature flag remains false. The checked-in candidate has a visible final order review, a non-personal Stripe payment description, the accepted receipt-email gate, the reviewed deletion and accounting boundary, and the 90-day minimal HMAC retention needed to prevent a completed Share Studio trial from being reacquired after account deletion. A rollout-hidden Share Studio Daily Snapshot creates its PNG entirely in the browser and sends only the existing opaque account session plus a random request ID to separately gated reserve/complete/release endpoints; connection credentials, glucose values, and the image do not go to Plus D1. The schema, production account/recovery lifecycle, stopped-environment lifecycle, and closed Share Studio browser/Worker/D1 lifecycle are accepted. Public account access and sales remain no-go until the remaining release candidates, service-binding cutover, feature-gating, final browser checks, and recorded rollback points pass.
