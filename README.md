...
## Plus 30-day pass — stopped production and non-public staging checkpoint
...
- On 2026-08-19, Stripe live mode created a separate active `GlucoScope Plus 30日パス` Product (`prod_V6ASxKCkGvR0Cs`) with one default JPY 400 one-time Price (`price_1U5y7tQk6xCYKhx8v3S5tn8j`). It is a one-time Price with no recurring interval or subscription. Production code now binds Checkout, returned Session facts, and signed webhooks to an explicit live/test mode, but every public Checkout, webhook, purchase, entitlement, sales, tax, and receipt gate remains off.
...
- Both APAC D1 databases now exist. `glucoscope-plus-staging` and `glucoscope-plus-production` have migrations `0001` through `0007`; all 13 application tables in each were verified at zero rows. Production stopped Version `12857f78-e233-4159-84bc-78c3fa56c76a` is deployed alone at 100% with the production D1 and account rate-limit bindings, dedicated production Turnstile, Resend, restricted live Stripe, Stripe webhook-signing, and two account-HMAC Secret bindings, explicit Stripe live mode, the exact live Product/Price IDs, and the approved JPY 400 sale terms. Stripe has one enabled live, non-Connect webhook destination at `https://plus.glucoscope.app/v1/stripe/webhook`, pinned to API Version `2026-06-24.dahlia` and limited to the seven reviewed Checkout and refund events. Every account, cleanup, RPC, purchase, Checkout, webhook, sales, tax, and receipt flag remains false. The production-only Custom Domain has no Cron or `workers.dev` target and returns the stopped `503` response with `no-store`. A Dashboard Secret update briefly deployed candidate-derived Version `7e823112-d8c1-4f8d-b563-101641169ce8` with account HTTP enabled while every commerce flag remained false; it was detected, replaced first by the prior stopped Version, and then by reviewed stopped Versions. A post-correction read found all 13 production application tables at zero rows. Version `12857f78-e233-4159-84bc-78c3fa56c76a` is the only reviewed production rollback point; Version `0cbbc227-6da0-41c3-9fd5-f280cd291152`, Version `7e823112-d8c1-4f8d-b563-101641169ce8`, unserved Secret Version `0b4842a9-9a62-4705-8358-612f926ad767`, and every earlier production Version must not receive traffic.
