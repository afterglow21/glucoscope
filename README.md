...
- Both APAC D1 databases now exist. `glucoscope-plus-staging` and `glucoscope-plus-production` have migrations `0001` through `0007`; all 13 application tables in each were verified at zero rows. Production stopped Version `c3a49718-e961-4557-b2bf-5f9224e8225f` is deployed at 100% with the production D1 binding, newly generated email-lookup and verification-code HMAC Secret bindings, every release flag false, no route, no Cron, and no `workers.dev` target. It is the only reviewed production rollback point. Earlier production Versions, including the unserved unusable-key Version `7dc440a4-6fc5-4907-8590-9c2b3ef97b3f`, must not receive traffic.
