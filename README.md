...
- Both APAC D1 databases now exist. `glucoscope-plus-staging` and `glucoscope-plus-production` have migrations `0001` through `0007`; all 13 application tables in each were verified at zero rows. Production Worker Version `d6c5de68-bdc5-41c0-8c30-2e2f5de74b33` is deployed at 100% with the production D1 binding, every release flag false, no Secret bindings, no route, no Cron, no preview URL, and no `workers.dev` target. It is the only production rollback point until a later stopped Version is explicitly accepted.
