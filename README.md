...
- Its disabled hourly cleanup foundation removes verification-code rows after they have been expired for 24 hours and global send-attempt reservations after they are 24 hours old. Once accepted and enabled with D1, the hourly schedule means those temporary records remain for about one day (normally no more than about 25 hours past the relevant boundary). Request-code and verify routes also have separate Cloudflare per-connection limits of 5 and 30 attempts per 60 seconds; the connecting IP is used only as the limiter key and is not saved to D1 or application logs. Verified-account, purchase, and accounting retention remains a separate unresolved decision.
...
- `auth.glucoscope.app` is verified for sending in Resend after the required SPF, DKIM, MX, and DMARC records were added manually in Cloudflare DNS and resolved publicly. Receiving is off and open/click tracking is not configured. No Resend API key, Worker Secret, real email, or deployment has been used.
- No Plus D1 binding, Resend API Secret, Stripe key, Webhook Secret, Price/Product identifier, or live purchase path is configured. Every account, purchase, quota, and feature switch remains `false`, so the public routes fail closed and no email or payment request can start. Account and sales flags remain off until a real closed email test and explicit acceptance of the ordinary 30-day retention and longer suppression-list exception pass.
