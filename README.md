...
7. Current: complete the general-user relay period, expiry, deletion, and limit acceptance before any Friends & Family continuing-enablement decision. Keep `RELAY_ENABLED=false` outside separately approved temporary test windows.
8. Non-blocking later: exercise demo-snapshot natural expiry in a deliberately stopped/failure-path window; healthy five-minute refreshes reset the 36-hour TTL, so expiry is not expected during normal continuous operation.
9. Return to Worker usage-counter and Usage Dashboard production verification, the site-wide Trust/About review, feedback, and the first announcement.
