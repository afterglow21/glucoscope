...
The comparison lab is published, but it currently falls back to the clearly labelled synthetic dataset because the dedicated Libre public-demo feed is paused. The Libre feed has completed one scheduled Worker retrieval and sanitized public-response check and was returned to the stopped Version. Guardian's separate Azure Nightscout browser route is verified, but that does not make the combined comparison page live by itself. The working frontend now configures the stopped G7 endpoint with `dexcomRouteVerified=false`; GitHub Pages verification remains pending until that frontend change is published. The lab does not rank devices, claim accuracy, select a reference CGM, or support treatment decisions. PR #14 merged the comparison frontend to `main` in merge commit `7e96648c27ce20fabe2f283c384124e36ce0b2d2`. GitHub Pages publication was verified on 2026-08-07 after the official deployment-lag incident was mitigated: `/demos/cgm-comparison/` loaded with the clearly labelled `準備中 · 合成データ` fallback and all three device cards.
...
2. Current: publish the configured stopped G7 frontend endpoint while keeping `dexcomRouteVerified=false`, then verify the synthetic fallback on GitHub Pages.
...
- General-user Gluroo connection details may stay in the selected browser and pass transiently through the limited relay. They are not stored in Azure, KV, Durable Objects, relay logs, shared cache, or AI. The separate public demo feed is limited to Kazuma's explicitly consented Libre and G7 demo values and never receives a general user's connection details or glucose data. The Libre and G7 slots have each completed one temporary live Worker check and have been returned to the stopped Version. The working frontend configures the stopped G7 URL with `dexcomRouteVerified=false`, and its Pages fallback check remains pending until publication.
