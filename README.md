...
The public site is served by GitHub Pages at `https://glucoscope.app/`. The custom domain is verified, HTTPS is enforced, and frontend commit `64a92932a592dda1b6eb9d6dd7700279b1c7a47a` is the accepted long-lived-session release checkpoint.
...
- A follow-up on the same iPhone fully closed the existing Home Screen app from the app switcher and reopened it. The G7 connection was still available without reconnecting, but changing only `apple-mobile-web-app-status-bar-style` from `black-translucent` to `black` did not remove the iOS status-bar overlap from that already installed icon. This is not an accepted display fix. Do not ask the person to delete or re-add the icon during verification, because doing so may remove the Home Screen app's locally saved URL, passphrase, or session context. This change adds an iOS Home Screen safe-area correction; one more real-device reopen remains required before accepting the display fix.
