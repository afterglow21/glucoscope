...
The comparison lab is published and currently shows the clearly labelled synthetic dataset because the dedicated public-demo Worker is paused. After the earlier stopped-endpoint fallback checks, live Version `4069bca4-e8cf-474a-9e9d-d7ffa42b7567` was deployed temporarily as deployment `9738343a-fc1d-4f02-aff1-1bae3d7cbe57` at 20:58:02 JST. The sanitized public responses contained 527 Libre entries and 276 G7 entries and passed the reviewed allowlist, type, range, ordering, recency, and CORS checks without printing glucose values, measurement timestamps, source credentials, or Secret values. GitHub Pages then displayed Guardian, Libre, and G7 as live with all three cards enabled, and Kazuma visually confirmed the three plotted lines. Stopped Version `9994a142-a4ca-4885-9077-952ec8e7e8d2` was restored as deployment `e45b6547-33a4-4196-9efe-1fffd412bcd4` at 21:16:31 JST. Both source routes returned `503`, and a newly opened page returned to the synthetic dataset. The lab does not rank devices, claim accuracy, select a reference CGM, or support treatment decisions.
...
After separate operational approval, live Version `4069bca4-e8cf-474a-9e9d-d7ffa42b7567` was deployed at 100% as deployment `9738343a-fc1d-4f02-aff1-1bae3d7cbe57` at 20:58:02 JST. Aggregate-only checks passed for 527 Libre entries and 276 G7 entries, including the reviewed fields, types, ranges, ordering, recency, and exact CORS. No glucose value, measurement timestamp, Gluroo URL, Secret, or token was printed or added to Git. GitHub Pages showed Guardian, Libre, and G7 as live with all three cards enabled, and Kazuma visually confirmed the three lines. This is one public-page acceptance; continued operation, repeated browser display refreshes, stale behavior, and natural expiry remain unverified. The review window crossed scheduled triggers, so no exact scheduled-refresh count is claimed.

Stopped Version `9994a142-a4ca-4885-9077-952ec8e7e8d2` was restored at 100% as deployment `e45b6547-33a4-4196-9efe-1fffd412bcd4` at 21:16:31 JST. Both public routes returned `503`, and a newly opened GitHub Pages view returned to the clearly labelled synthetic dataset. After the 21:25 JST stopped Cron, metadata showed only the two expected source keys, no metadata payload, and unchanged expirations for both keys. This confirms that the stopped Cron did not refresh either snapshot or extend either expiration. `dexcomRouteVerified=true` remains in the frontend as a display-path verification record; it is not a Worker enable switch. The general-user Limited Data Relay remains stopped.
...
- General-user Gluroo connection details may stay in the selected browser and pass transiently through the limited relay. They are not stored in Azure, KV, Durable Objects, relay logs, shared cache, or AI. The separate public demo feed is limited to Kazuma's explicitly consented Libre and G7 demo values and never receives a general user's connection details or glucose data. The public demo has completed one three-source GitHub Pages live acceptance and has been returned to the stopped Version. The frontend keeps `dexcomRouteVerified=true` as a record of that verified display path, but this flag does not enable the Worker; a newly opened or reloaded public page falls back to labelled synthetic data while the Worker is stopped.
