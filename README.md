...
- Code checkpoint `dabb3571` is deployed only as the stopped `glucoscope-plus-entitlement-staging` Worker. Version `c917affd-74ed-4691-a3c6-b6c8e3149e3c` receives 100% of that Worker's traffic, but `workers_dev=false`, preview URLs are off, there are no routes or Cron triggers, observability is off, and the `workers.dev` URL returns `404`. Six encrypted test-only Secret bindings remain available for later closed acceptance; their values are not recorded. Every account, cleanup, RPC, purchase, Checkout, webhook, sales-readiness, and tax-readiness switch is `false`.
...
- The Plus D1 binding exists only in the stopped staging environment. Test Product/Price identifiers and encrypted restricted-key/webhook bindings are retained for further closed acceptance, but the Stripe webhook destination is disabled, its temporary Custom Domain has been deleted, and there is no public account route, Checkout, Cron, or live purchase path. Every public account, purchase, quota, and feature switch remains `false`, so no public email or payment request can start. Public account access and sales remain no-go until the retention exception, recovery and delivery-failure procedures, receipt wording, professional review, remaining payment cases, and production acceptance pass.
