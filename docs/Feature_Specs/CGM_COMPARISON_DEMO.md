...
This architecture does not use or control the general-user Limited Data Relay. That separate relay is currently enabled only for the approved 1–3 person early-access group, while its checked-in `RELAY_ENABLED=false` safety default and no-storage boundary remain unchanged.
...
- Dexcom G7 readings are confirmed in Gluroo. Its dedicated public feed is live in the continuous comparison and has passed repeated scheduled aggregate checks, about three hours of continued operation, and two browser auto-refresh checks at separate checkpoints. The frontend retains `dexcomRouteVerified=true` as a display-path verification record, not as a Worker enable switch. Separately, on 2026-08-12 the general-user Limited Data Relay G7 path passed a supervised iPhone Safari acceptance for connection, current glucose, today/yesterday/7-day/30-day graph periods, reload, and connection deletion returning to setup. The relay was then returned to `RELAY_ENABLED=false`; natural ticket expiry, persistence after fully quitting Safari, and live request-limit exhaustion were not part of that acceptance.
...
25. Completed on 2026-08-12: verify the general-user Limited Data Relay G7 connection, current glucose, today/yesterday/7-day/30-day graph periods, reload, and connection deletion in iPhone Safari, then return that relay to `RELAY_ENABLED=false`.
26. Current project step after separate explicit approval: keep the general-user Limited Data Relay enabled only for the 1–3 person early-access group, with the reviewed stopped Version ready for immediate rollback. Natural ticket expiry, persistence after fully quitting Safari, and live request-limit exhaustion remain operational observations.
27. Non-blocking later: exercise demo-snapshot natural expiry during a deliberately stopped/failure-path window. Healthy five-minute refreshes reset the 36-hour TTL, so natural expiry is not expected during normal continuous operation.
28. Return the affected source or the entire Worker to stopped Version `9994a142-a4ca-4885-9077-952ec8e7e8d2` immediately if consent, privacy, traffic, terms, safety, or data-quality conditions require it.
