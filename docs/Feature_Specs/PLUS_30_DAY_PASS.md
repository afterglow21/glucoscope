...
Status: live public beta release / JPY 400 one-time product / 30 days / no automatic renewal / adult-or-guardian buyer / Japan-only sale / tax-exempt seller status / public support and conditional full-refund policy / live payment, refund, receipt, account recovery, deletion, retention, and Share Studio acceptance passed
...
価格と主な機能、販売地域、税務状態、公開問い合わせ先、返金方針、保持・削除方針と運営手順は決定済みである。Stripe test modeの異常系に加え、本番の400円決済、全額返金、支払・返金メール、同じメールへの復旧、古いsessionの失効、アカウント削除、Share Studioの1回体験と再利用拒否まで受け入れを完了した。2026年8月20日に小規模な一般提供を開始し、2026年8月23日に実決済と主要特典の最終受入後、GlucoScope公開ベータと同時に最初のお知らせを開始した。追加の支払方法は初期販売で有効にしない。専門家確認と法令・税務条件の見直しは公開後も続け、条件が変わった時は販売を止めて表示と運用を更新する。
...
| PlusのAI分析 | 認証済みPlusアカウントにつき、通常画面のやさしい分析・しっかり分析を合わせて1日5回まで。Share Studioも別枠で1日5回まで。どちらも成功した新規分析だけを数え、失敗と保存済み表示は回数外 |
...
- 成功した新規AI分析を、通常画面のやさしい分析・しっかり分析で1日5回まで
- Share Studioで使うやさしい分析を、通常AIとは別に1日5回まで
...
Plusの「1日5回」は、認証済みPlusアカウント単位とする。ブラウザや端末を変えても同じアカウントの回数として扱う。通常画面のやさしい分析・しっかり分析と、Share Studioの3枚目に使うやさしい分析は同じ5回を共有する。Share Studio内部で表示品質を整えるために書き直しても、表示できる最終文面が確定した時だけ1回と数え、すべて失敗した時は0回とする。ブラウザから送られた「Plusです」という申告だけを信用せず、サーバー側で有効な利用権を確認する。
...
Share Studioは、本人が選んだ時だけ、今日出逢ったグルコ、日次指標と血糖グラフ、やさしいAIふりかえり、締めくくりの4枚を端末で作る。1枚目のグルコは別の選択肢から選ばせず、その日にGlucoScopeが端末内へ記録した実際の出逢いを使う。無料体験中のAI生成は、認証済みアカウントのその1回の有効な体験予約にだけ結び付け、通常Freeの1日1回枠とは別の体験専用1回枠として扱う。通常のFree画面へしっかり分析を開放しない。SNSへの自動投稿は行わず、共有前に健康情報を含むことを明示し、端末の共有機能へ渡すところまでとする。接続URL、合言葉、不要な個人情報を画像へ含めない。

無料体験は、認証済みアカウントごとに4枚の完成画像を1組作れるものとする。プレビュー表示、説明を読むこと、失敗した作成は消費しない。4枚すべてをブラウザのIndexedDBへ保存し、読み戻せることを確認した後だけ使用済みにし、その画面で「無料体験1回分を使用した」と明示する。同じrequest IDやWebhookで二重消費しない。作った4枚は、Share Studio内の削除またはブラウザデータ削除まで同じ端末から再表示・再保存できる。画像、血糖値、接続先URL、合言葉はPlus Workerへ送らない。3枚目の生成には日次の限定サマリーだけをAI Workerへ送る。端末変更やブラウザデータ削除を越える同期・復旧は行わない。
...
> Plus 30日パスは400円の1回払いです。購入した時から30日間、通常画面のやさしい分析としっかり分析を合わせて1日5回まで、Share Studioも別枠で1日5回まで使えます。しっかり分析のグルコのお話し・AIお手紙・ChatGPTに相談、グラフの7日・30日・カスタム期間も使えます。自動では更新されません。血糖値を見る基本機能、今日・昨日のグラフ、1日1回のやさしい分析は、Plusに入らなくても使えます。
...
5. Stripeへ移動する前の最終確認画面、固定の非個人決済説明、領収書の独立した停止ゲートは実装した。Stripeの公開問い合わせ先、既定メール言語、事業サイト、事業説明を現在のPlus仕様へ同期し、sandbox返金領収書と本番の成功決済・返金メールで、日本語、総額400円、公開問い合わせ先、現在の事業サイトを確認した。本人受信箱への到着も確認済みで、2通目が迷惑メールへ分類された実例は不達案内へ反映した。checked-in設定はfail-closedのため`false`を維持し、実際の本番は受入済みの明示overrideだけで領収書gateを有効にする
6. 免税事業者としてStripe Taxの自動計算を使わず、お支払い総額400円、適格請求書を発行しない初期表示、通常の支払確認・領収書を実環境で受け入れた。事業形態、登録、税務条件が変わった時はCheckoutを止め、専門家または利用する会計手順で再確認してから表示と設定を更新する
