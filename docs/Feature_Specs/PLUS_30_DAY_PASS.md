...
Status: approved core product, adult-or-guardian buyer, Japan-only sale, JPY 400 total, tax-exempt seller status, public support, conditional full-refund policy, operator runbook, and first Stripe full-payment/refund drill / receipt, deletion, retention, professional, remaining payment-case, and implementation acceptance pending / stopped non-public staging only / not available for purchase
...
価格と主な機能、販売地域、税務状態、公開問い合わせ先、返金方針と運営手順は決定済みで、Stripe test modeの最初の決済・重複通知・全額返金ドリルも完了した。ただし販売はまだ開始しない。本人または保護者の追加受け入れと復旧、領収書、保持期間、専門家確認、残る決済ケース、安全確認を終えるまで購入導線を公開しない。
