...
Share Studioは、本人が選んだ時だけ、今日出逢ったグルコ、日次指標と血糖グラフ、やさしいふりかえり、締めくくりの4枚を端末で作る。1枚目のグルコは別の選択肢から選ばせず、その日にGlucoScopeが端末内へ記録した実際の出逢いを使う。SNSへの自動投稿は行わず、共有前に健康情報を含むことを明示し、端末の共有機能へ渡すところまでとする。接続URL、合言葉、不要な個人情報を画像へ含めない。

無料体験は、認証済みアカウントごとに4枚の完成画像を1組作れるものとする。プレビュー表示、説明を読むこと、失敗した作成は消費しない。4枚すべてをブラウザのIndexedDBへ保存し、読み戻せることを確認した後だけ使用済みにする。同じrequest IDやWebhookで二重消費しない。作った4枚は、Share Studio内の削除またはブラウザデータ削除まで同じ端末から再表示・再保存できる。画像、血糖値、接続先URL、合言葉はPlus Workerへ送らない。端末変更やブラウザデータ削除を越える同期・復旧は行わない。
