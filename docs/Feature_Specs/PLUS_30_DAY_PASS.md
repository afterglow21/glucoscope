...
Share Studioは、本人が選んだ時だけ、今日出逢ったグルコ、日次指標と血糖グラフ、しっかりAI分析、締めくくりの4枚を端末で作る。1枚目のグルコは別の選択肢から選ばせず、その日にGlucoScopeが端末内へ記録した実際の出逢いを使う。無料体験中のしっかりAI分析は、認証済みアカウントのその1回の有効な体験予約にだけ結び付け、通常のFree画面へしっかり分析を開放しない。SNSへの自動投稿は行わず、共有前に健康情報を含むことを明示し、端末の共有機能へ渡すところまでとする。接続URL、合言葉、不要な個人情報を画像へ含めない。
