...
Status: approved core product, adult-or-guardian buyer, Japan-only sale, JPY 400 total, tax-exempt seller status, public support, conditional full-refund policy, operator runbook, and closed Stripe payment/refund/concurrency/expiry/decline drills / receipt, deletion, retention, professional, additional-payment-method, and implementation acceptance pending / stopped non-public staging only / not available for purchase
...
価格と主な機能、販売地域、税務状態、公開問い合わせ先、返金方針と運営手順は決定済みで、Stripe test modeの決済、重複通知、全額返金、二重操作、未完了Checkoutの再利用、期限切れ、再作成、カード拒否ドリルと、同じメールへの復旧・古いsessionの失効・アカウント削除も完了した。ただし販売はまだ開始しない。本人または保護者の追加受け入れ、メール配信失敗時の手順、領収書、保持期間、専門家確認、追加で有効にする支払方法、安全確認を終えるまで購入導線を公開しない。
...
1. Resendで認証済みの `auth.glucoscope.app` と公式テスト宛先による受理・`delivered`確認、本人受信箱と実Turnstileを含む最初の非公開E2E、同じメールへの2回目の送信・復旧、古いsessionの失効、新しいsessionの継続、アカウント削除は完了した。遅延時の利用者案内、60秒の再送待機、最新コードへの切替、失敗した再送で旧コードを失わない実装と運営手順も追加した。開封・クリック追跡を無効にしたまま、次は追加の少人数受信箱と、`delivery_delayed`・bounce・complaint・suppressionの実地運用を受け入れる。無料枠、通常最長30日の保持、Suppression Listへ30日を超えて残り得る例外、原因解決後に手動削除する運用を追加送信前に再確認する
...
- その後の再送安全性候補の受け入れは、`403 turnstile_failed`でD1やメールの前に停止した。繰り返し操作してもaccount、challenge、送信予約、session、購入、利用権は作られず、メール到着の証拠も採用していない。localhostのremote-dev service-binding bridgeも、有効な診断requestの中継中に失敗したため、この経路を再送修正の受け入れ証拠にしない。harnessと診断copyを削除し、12個のapplication tableがすべて0件、`workers.dev`が`404`であることを再確認した。現在は新しい停止Version `29574f7c-d449-4a99-8e50-d4862b0d6d33`だけを100%へ向け、全flag `false`、route/Cron/previewなしを維持する。4つのSecret binding名は保持するが、値は次の試験前に開示せず再設定または再検証する。再送修正はローカルテスト済みだが、実環境E2Eは未完了である。
