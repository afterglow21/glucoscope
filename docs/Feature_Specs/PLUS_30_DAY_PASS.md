...
- 確認コードの一時記録を期限後24時間超、全体送信予約を送信試行後24時間超で削除する毎時cleanupを、停止flag、D1 binding、失敗時の再確認とともに実環境で受け入れている
