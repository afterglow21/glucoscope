...
A follow-up on the same iPhone fully closed the existing Home Screen app from the app switcher and reopened it. The G7 connection remained available without reconnecting, but the iOS status bar still overlapped the GlucoScope header after the published metadata-only change from `black-translucent` to `black`. The metadata-only change is therefore not an accepted display fix. During this verification, do not ask the person to delete or re-add the icon: removing the Home Screen app may remove its locally saved URL, passphrase, or session context. Keep the existing icon and connection in place. This change adds an iOS Home Screen safe-area correction; one more real-device reopen remains the only open display acceptance item.
...
同じiPhoneで、既存のホーム画面アプリをアプリ切り替え画面から完全に終了し、同じアイコンから開き直しました。G7接続は再接続なしで維持できましたが、公開済みの `apple-mobile-web-app-status-bar-style` を `black-translucent` から `black` へ変えるだけの修正では、iOSステータスバーとGlucoScopeヘッダーの重なりは解消しませんでした。このmeta変更だけでは表示修正の受入完了としません。確認中はアイコンの削除や再追加を案内しません。ホーム画面アプリを削除すると、アプリ内だけに保存した接続先URL、合言葉、session contextを失うおそれがあるためです。この変更で既存アイコンと接続を維持したままiOSホーム画面用のCSS safe area修正を追加し、残る表示受入は同じ手順での実機再確認だけとします。
