...
test("public relay wording preserves the current verification and privacy boundaries", async () => {
  ...
  assert.match(data, /短時間の受け入れで、Guardian・Libre・G7の3つのライブカードと3本のグラフをGitHub Pages上で確認しました/);
  assert.match(data, /停止中のG7接続先は`dexcomRouteVerified=false`のままGitHub Pagesへ反映し、合成データへの切替を確認済みです/);
  assert.match(data, /確認後は停止Versionへ戻し、新しく開いたページが「準備中・合成データ」へ戻ることも確認済みです/);
  ...
  assert.match(roadmap, /公開3CGM比較ラボの短時間ライブ受け入れと停止復帰は完了し、現在は古いデータ・自然失効の確認と、継続公開の別判断が残っています/);
  ...
  assert.match(roadmap, /完了：停止Versionへ戻し、両経路の停止応答と新しく開いたページの合成データへの復帰を確認しました/);
  assert.match(roadmap, /現時点の3CGM比較ページは、公開デモ用Workerが停止中のため「準備中・合成データ」を表示しています/);
  assert.match(roadmap, /短時間の受け入れ中には、GuardianをKazumaの既存Azure Nightscoutからブラウザで直接読み/);
  ...
  assert.match(roadmap, /The brief live acceptance and stopped-state restoration of the public 3CGM Comparison Lab are complete/);
  assert.match(roadmap, /Guardian, Libre, and G7 were briefly displayed together as three live cards and three graph lines/);
  ...
  assert.match(readme, /currently shows the clearly labelled synthetic dataset because the dedicated public-demo Worker is paused/);
  assert.match(readme, /public-demo Worker first completed source-specific G7 and Libre checks, then completed one approved Guardian\/Libre\/G7 public-page acceptance/);
  ...
  assert.match(readme, /published frontend now keeps `dexcomRouteVerified=true` because the G7 display path passed once; this frontend flag does not enable either Worker route/);
  assert.match(readme, /live Version `4069bca4-e8cf-474a-9e9d-d7ffa42b7567` was deployed temporarily as deployment `9738343a-fc1d-4f02-aff1-1bae3d7cbe57` at 20:58:02 JST/);
  ...
  assert.match(readme, /This is one public-page acceptance; continued operation, repeated browser display refreshes, stale behavior, and natural expiry remain unverified/);
