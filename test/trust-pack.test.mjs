...
test("public relay wording preserves the current verification and privacy boundaries", async () => {
  ...
  assert.match(privacy, /接続確認は成功しましたが/);
  assert.match(privacy, /利用プロフィールは作成されていません/);
  assert.match(privacy, /06aa2dbe-454b-45b8-859a-d8e5b9741a82/);
  assert.match(privacy, /USAGE_COLLECTION_ENABLED=false/);
  assert.match(privacy, /公開フロントの一時確認用gateは <code>true<\/code> のままですが、Workerは停止中です/);
  assert.match(privacy, /接続保存と待ち時間の修正はWorker停止のまま公開版へ反映し/);
  ...
  assert.match(roadmap, /接続確認は成功しましたが/);
  assert.match(roadmap, /既知の試験用プロフィール2件も削除しました/);
  assert.match(roadmap, /<code>0 \/ 0 \/ 0<\/code> のままで、利用プロフィールは作成されていません/);
  assert.match(roadmap, /5d160aed-7b27-48e6-b0a8-783534f97b6f/);
  assert.match(roadmap, /06aa2dbe-454b-45b8-859a-d8e5b9741a82/);
  assert.match(roadmap, /Worker停止のまま公開版へ反映済みです/);
  assert.match(developerStatus, /06aa2dbe-454b-45b8-859a-d8e5b9741a82/);
  assert.match(developerStatus, /Workerは停止中です/);
  ...
  assert.match(roadmap, /The public frontend now makes connection storage robust/);
