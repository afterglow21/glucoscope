...
test("public relay wording preserves the current verification and privacy boundaries", async () => {
  ...
  assert.match(privacy, /503 usage_collection_paused/);
  ...
  assert.match(roadmap, /再コールバック防止と簡素な画面はローカル実装済みです/);
  assert.match(roadmap, /既知の試験用プロフィール2件は、その後削除し/);
  assert.match(roadmap, /D1が再確認でも <code>0 \/ 0 \/ 0<\/code>/);
  ...
  assert.match(roadmap, /The callback guard and simplified screen are implemented locally/);
