...
test("public relay wording preserves the current verification and privacy boundaries", async () => {
  ...
  assert.match(data, /フロント接続、GitHub Pages上の表示、3機種の同時比較、ページ全体のエンドツーエンド経路は未確認です/);
  ...
  assert.match(data, /G7フロント接続先は空のままです/);
  ...
  assert.match(privacy, /フロント接続、GitHub Pages上のG7表示、3機種同時比較、ページ全体の経路は未確認です/);
  ...
  assert.match(readme, /frontend G7 endpoint remains blank and `dexcomRouteVerified=false`/);
  assert.match(readme, /GitHub Pages G7 rendering, simultaneous three-source comparison, repeated scheduled refreshes, and continuing enablement remain unverified/);
