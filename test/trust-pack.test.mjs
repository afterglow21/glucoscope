...
test("public relay wording preserves the current verification and privacy boundaries", async () => {
  ...
  assert.match(data, /Dexcom G7はGlurooまで確認済みですが、GlucoScopeまでのライブ経路を確認済みとは案内しません/);
  ...
  assert.match(data, /Dexcom G7からGlurooまでの表示に加え、停止中の公開デモ用Workerルートと安全境界も確認済み/);
  assert.match(data, /ライブ取得、KVスナップショット、フロント接続、公開表示までの全経路は未確認です/);
  assert.match(data, /G7の血糖値と測定・更新時刻の公開へも、Libreとは別に明示同意しました/);
  assert.match(data, /停止multi-source Version `9994a142-a4ca-4885-9077-952ec8e7e8d2`は、本番通信の100%へ反映済みです/);
  assert.match(data, /`DEMO_FEED_ENABLED=false`、`DEMO_LIBRE_FEED_ENABLED=false`、`DEMO_G7_FEED_ENABLED=false`の3つのゲート/);
  assert.match(data, /`GLUROO_DEMO_SOURCE_URL`、`GLUROO_DEMO_API_SECRET`、`GLUROO_DEMO_G7_SOURCE_URL`、`GLUROO_DEMO_G7_API_SECRET`の4つのSecret名/);
  assert.match(data, /Secret値は画面出力やGitへ入れず、一時ログは安全確認後に削除しました/);
  assert.match(data, /`\/v1\/libre`と`\/v1\/dexcom-g7`のGETはいずれも停止中の`503`/);
  assert.match(data, /許可Originのpreflightが`204`、不許可Originが`403`/);
  assert.match(data, /5分Cron後もKVは空/);
  assert.match(data, /この停止確認ではGlurooから取得せず、KVへ書き込まず、G7のフロント接続先を有効化していません/);
  ...
  assert.match(readme, /dedicated KV remained empty after a Cron boundary/);
  ...
  assert.match(readme, /comparison frontend still uses only the stopped `\/v1\/libre` endpoint/);
  ...
  assert.match(readme, /No G7 KV value was written, no live Gluroo retrieval occurred/);
