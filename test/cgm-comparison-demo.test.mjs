...
test("public sample is explicitly synthetic, three-source, and privacy-safe", async () => {
  ...
  assert.match(dataset.disclosure, /3機種の実測ライブ表示は1回確認後/);
  assert.match(dataset.disclosure, /公開デモWorkerを停止/);
  ...
  assert.match(dataset.sources[1].captureRoute, /公開デモ用Worker、GitHub Pagesまで1回確認済み/);
  assert.match(dataset.sources[1].note, /3機種同時の実測ライブ表示を1回確認済み/);
  assert.match(dataset.sources[1].note, /現在はWorker停止中/);
  assert.match(dataset.sources[1].note, /継続運用、複数回のブラウザ表示更新、古いデータ・自然失効は未確認/);
  assert.match(dataset.sources[1].note, /表示中の線は合成データ/);
  assert.equal(dataset.sources[2].verificationLabel, "公開デモ経路を実機確認済み");
  assert.match(dataset.sources[2].captureRoute, /公開デモ用Worker、GitHub Pagesまで1回確認済み/);
  assert.match(dataset.sources[2].note, /一般利用者向け限定中継とは別/);
  assert.match(dataset.sources[2].note, /3機種同時の実測ライブ表示を1回確認済み/);
  assert.match(dataset.sources[2].note, /現在はWorker停止中/);
  assert.match(dataset.sources[2].note, /dexcomRouteVerified=true/);
  assert.match(dataset.sources[2].note, /Workerの有効化ではありません/);
  assert.match(dataset.sources[2].note, /継続運用、複数回のブラウザ表示更新、古いデータ・自然失効は未確認/);
...
test("live comparison uses Guardian and Libre while Dexcom remains pending", () => {
  ...
  assert.match(dataset.sources[1].note, /3機種同時ライブ表示を1回確認済み/);
  assert.match(dataset.sources[1].note, /継続運用、複数回のブラウザ表示更新、古いデータ・自然失効は未確認/);
  assert.deepEqual(dataset.sources[2].readings, []);
  assert.equal(dataset.sources[2].verificationLabel, "公開デモ経路を実機確認済み");
  assert.match(dataset.sources[2].captureRoute, /公開デモ用Worker、GitHub Pagesまで1回確認済み/);
  assert.match(dataset.sources[2].note, /一般利用者向け限定中継とは別/);
  assert.match(dataset.sources[2].note, /3機種同時ライブ表示を1回確認済み/);
  assert.match(dataset.sources[2].note, /継続運用、複数回のブラウザ表示更新、古いデータ・自然失効は未確認/);
